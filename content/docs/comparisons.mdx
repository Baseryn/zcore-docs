...
from zcore import BaseRepository, get_db
...
ZCore does not replace SQLAlchemy; it builds on top of it. However, it is important to understand the level of abstraction introduced by `BaseRepository`.
...
| **`Inject()`** | Requires understanding container lifecycles (Singleton, Scoped, Transient). | Standard FastAPI `Depends()` parameters meet your modular requirements. |
