...
| **Data Access (CRUD)** | Manual SQL or ORM execution per route | Automated inheritance via `BaseRepository` (with dialect-aware `RETURNING` fallback & model/ID targets) |
...
| **Soft Delete** | Manual query filtering per route | Built-in `SoftDeleteMixin` with timezone-aware timestamps and automatic `scope_query` integration |
| **Structured Logging** | Manual configuration | Multi-tier `setup_logging` with `LoggingSettings`, rotating file logs, and query timing |
...
| **Mutations (Update/Delete)** | Requires query execution by primary key | `repo.update(target, schema)`, `repo.delete(target)` (accepts model instance or ID) |
| **Bulk Insert / Update / Delete** | Manual `insert().values().returning()` setup | `repo.create_multi()`, `repo.update_multi()`, `repo.delete_multi()` with dialect-aware `RETURNING` fallback |
...
| **Search Operators** | Manual operator translation | Built-in `between`, `contains`, `startswith`, `endswith`, `ilike`, `in`, `is_null` with non-string type casting |
| **Search Security** | Manual validation required | Configurable `__max_search_depth__` and restricted field checking |
| **Execution Hooks** | Hand-written or mapped via event listeners | Overridable `pre_create`, `pre_update`, `post_delete` single/bulk service hooks |
