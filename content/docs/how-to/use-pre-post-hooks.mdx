...
<Steps>
  ...
  <Step>
    ...
    class TaskService(BaseService[Task]):
        ...
        async def on_delete(self, target: Task | Any) -> Task | None:
            # Example: Soft-delete override instead of permanent row deletion
            return await self.repository.update(target, TaskUpdate(is_deleted=True), partial=True)
    ```
  </Step>
  ...
  <Step>
    ...
    | `pre_delete(target)` | Delete (Single) | `None` | Verify dependencies or cascade rules before row removal. |
