...
@app.get("/")
async def root():
    return {"status": "healthy", "framework": "ZCore", "debug": settings.DEBUG}
```
...
Now, uncomment the router inclusion in `tasks/plugin.py`:
...
By simply defining this class, you gain access to async single & bulk CRUD operations (`create_multi`, `update_multi`, `delete_multi`), structured dynamic search filters (`contains`, `between`, `ilike`, etc.), and keyset cursor-based pagination.
...
Available lifecycle hooks include `pre_create`, `post_create`, `pre_update`, `post_update`, `pre_delete`, `post_delete`, and their `_multi` counterparts to cleanly separate business logic from routing layers.
...
When a user logs in, their profile maps restricted fields (e.g., `["tasks.assignee_email"]`) into `ctx.restricted_fields`. `Zchema` automatically intercepts serialization and prunes restricted attributes from responses, input validation, and OpenAPI documentation (`?schema=true`).
...
If any database statement fails inside the context manager, the entire transaction automatically rolls back and pending events are cleared.
