---
title: What is ZCore?
description: A modular architectural framework designed for FastAPI by Baseryn. Use an isolated component or adopt the full stack — your code stays standard FastAPI.
...
**FastAPI ZCore Framework is a high-performance modular architectural framework built on top of FastAPI, created by [Ali Alf Ostovar](https://github.com/alialfostovar) and maintained by [Baseryn](https://github.com/Baseryn).**

It provides a cohesive set of enterprise-grade design patterns—such as constructor dependency injection with auto-wiring, repository pattern abstractions with bulk mutation optimizations, dynamic schema projections (`Zchema`), transactional Unit of Work, and isolated background tasks—designed to eliminate repetitive boilerplate in growing, high-throughput FastAPI applications.
...
| **An enterprise monolith** | It includes Unit of Work and plugin management. | `UnitOfWork` is a lightweight context manager; `Plugin` is an optional startup registry with DAG topological sorting. Both function seamlessly even in a small, two-file project. |
...
| **All-or-nothing** | It carries the umbrella name of a framework. | You can import and use `Inject[T]`, `background_task`, or `BaseRepository` independently in a project that utilizes zero other ZCore modules. |
...
<Tabs items={['Repository Only', 'Service + Repository', 'Zchema with Plain Router', 'Isolated Background Tasks']}>
...
<Tab value="Isolated Background Tasks">
...
@background_task
async def send_invoice(order_id: uuid.UUID, service: Inject[InvoiceService]):
    # Executes with an isolated AsyncSession and IoC scope
    await service.process(order_id)
```
*Safe background execution without closed session leaks.*
...
| Enforce strict project layouts | The `zc` CLI provides an interactive, modular scaffolding wizard; you can adapt or delete it. |
| Manage database migrations | Use Alembic directly for full control over migration histories. |
| Provide built-in user authentication UI | It offers `BaseAuth` utilities and Argon2id hashing, but authentication state and models are yours to define. |
...
### ZCore helps when...
*   You are writing repetitive CRUD endpoints and bulk data-access queries across multiple models.
...
2. **Targeted Utilities** (Adopting `Inject[T]`, `background_task`, or `UnitOfWork`)
3. **Data Access Layer** (`BaseRepository`, `SoftDeleteMixin`, and `BaseService`)
4. **Security & Projection** (`Zchema` for role-based field filtering)
...
<Cards>
  <Card title="Inject & IoC Container" href="/docs/api-reference/ioc-container/">
    Explore automatic type-safe constructor auto-wiring and signature caching.
  </Card>
  <Card title="Background Tasks & Scopes" href="/docs/api-reference/background-tasks">
    Isolated AsyncSessions and IoC auto-wiring for background routines.
  </Card>
  <Card title="BaseRepository" href="/docs/api-reference/repository">
    Async CRUD, bulk RETURNING mutations, keyset pagination, and dynamic search.
  </Card>
  <Card title="BaseService" href="/docs/api-reference/service">
    Isolate domain business logic with pre/post execution hooks.
  </Card>
  <Card title="Zchema" href="/docs/api-reference/zchema">
    Dynamic field-level security pruning for responses, inputs, and OpenAPI schemas.
  </Card>
  <Card title="UnitOfWork" href="/docs/api-reference/uow">
    Atomic transactions and post-commit deferred event dispatching.
  </Card>
</Cards>