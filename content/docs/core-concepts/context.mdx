...
### Structured Settings Schemas
Settings in ZCore are structured into dedicated sub-models for core subsystems:
...
* **`LOGGING` (`LoggingSettings`)**: Manages log levels, formatters (`json_format`), SQL query logging (`log_sql_queries`), slow query thresholds (`slow_query_threshold_ms`), rotating log file destinations (`file_path`), and muted loggers (`muted_loggers`).
