...
2. **Expression Building & Type Coercion:** Recursively translates structured JSON operators (`eq`, `ne`, `gt`, `lt`, `ge`, `le`, `ilike`, `contains`, `startswith`, `endswith`, `in`, `between`, `is_null`, `and`, `or`) into SQLAlchemy binary comparison clauses while safely coercing values (`int`, `float`, `date`, `datetime`, `UUID`, `bool`, and `set`).
...
* **Filter Nesting Depth:** Recursive filter structures are limited to a default maximum depth of **3 levels** (configurable per model via `__max_search_depth__`). Exceeding this raises a `ValidationError("Search query filter structure is too complex.")`.
* **Relation Include Depth:** The `include` array is strictly bounded to the maximum depth limit (e.g., `["organization.owner.profile"]`).
...
## 5. Relational Dot-Path Resolution & Type Safety
...
<Callout type="info" emoji="⚡">
  ...
  * All text operators (`ilike`, `contains`, `startswith`, `endswith`) automatically escape SQL wildcard characters (`%`, `_`, `\`) to prevent query distortion or wildcard injection attacks.
